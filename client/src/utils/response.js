const response = {
  "nominalization": {
    "instances": [
      {
        "index": 83,
        "token": "perfection",
        "end": 432,
        "data": {
          "remove": false,
          "suggestion": "Use lively version of the word"
        },
        "start": 422
      },
      {
        "index": 86,
        "token": "illusion",
        "end": 447,
        "data": {
          "remove": false,
          "suggestion": "Use lively version of the word"
        },
        "start": 439
      },
      {
        "index": 112,
        "token": "perfection",
        "end": 584,
        "data": {
          "remove": false,
          "suggestion": "Use lively version of the word"
        },
        "start": 574
      },
      {
        "index": 125,
        "token": "ambition",
        "end": 660,
        "data": {
          "remove": false,
          "suggestion": "Use lively version of the word"
        },
        "start": 652
      },
      {
        "index": 133,
        "token": "rejection",
        "end": 701,
        "data": {
          "remove": false,
          "suggestion": "Use lively version of the word"
        },
        "start": 692
      }
    ],
    "description": "Complex nouns extended from shorter verbs, adjectives and nouns. They often slow a reader down and may be hard to interpret.  Try to rearrange the sentence to use the original shorter and more lively version of the word.",
    "title": "Nominalization"
  },
  "tasteandsmell": {
    "instances": [
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.803362491881076,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "pungent"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.78491413185435788,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "sugary"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.86806046961310002,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "tasteless"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.79884314637544784,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "fishy"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.79076480866365151,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "peppery"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.78600185895989361,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "odorous"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.86325603707739584,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "acrid"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.80387097119250817,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "musty"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.77784329565770738,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "flowery"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.81608080931849203,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "putrid"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.81830596832562852,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "earthy"
        },
        "start": 200
      },
      {
        "index": 45,
        "token": "compulsive",
        "end": 248,
        "data": {
          "similarity": 0.76344799981461142,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "acrid"
        },
        "start": 238
      },
      {
        "index": 45,
        "token": "compulsive",
        "end": 248,
        "data": {
          "similarity": 0.76167190138290486,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "putrid"
        },
        "start": 238
      }
    ],
    "description": "Taste and smell are closely related. Most taste and smell words are easy substitutes for bland words like good, nice, or bad. For instance: zesty, tantalizing, sweet, stinky, stale.",
    "title": "Taste and Smell Sensory Word"
  },
  "suggestions": {
    "instances": [
      {
        "start": 5,
        "token": "about",
        "end": 10,
        "data": {
          "remove": true,
          "suggestion": "Try not to use this term when discussing quantities. Use \"approximately\" or a range instead. Ex: About 20 people attended. Better: Approximately 20 people attended. Or: Fifteen to twenty people attended."
        }
      },
      {
        "start": 11,
        "token": "all things being equal",
        "end": 33,
        "data": {
          "remove": true,
          "suggestion": "Empty Phrase. Don't use it. Ex: All things being equal, we should arrive tonight. Better: If all goes well, we should arrive tonight."
        }
      },
      {
        "start": 99,
        "token": "you can",
        "end": 106,
        "data": {
          "remove": true,
          "suggestion": "Flabby verb helpers. Delete or revise. Ex: You can visit Oz by following the Yellow Brick Road. Better: To visit Oz, follow the Yellow Brick Road."
        }
      },
      {
        "start": 230,
        "token": "had",
        "end": 233,
        "data": {
          "remove": true,
          "suggestion": "Nominalization (wordiness introduced when someone uses the noun equivalent of a verb or adjective). Use the verb or adjective form for more powerful sentences. Ex: We had a conversation about money. Better: We talked money."
        }
      },
      {
        "start": 478,
        "token": "you can",
        "end": 485,
        "data": {
          "remove": true,
          "suggestion": "Flabby verb helpers. Delete or revise. Ex: You can visit Oz by following the Yellow Brick Road. Better: To visit Oz, follow the Yellow Brick Road."
        }
      }
    ],
    "description": "Various different suggestions to improve the prose of your writing",
    "title": "Common Suggestions"
  },
  "visual": {
    "instances": [
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.82275456289748572,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "hazy"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.78346115461886112,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "gloomy"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.82045120298536178,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "murky"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.77491581955361666,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "somber"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.75608003363223508,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "flamboyant"
        },
        "start": 200
      }
    ],
    "description": "Words related to sight indicate colors, shape, or appearance. For instance: gloomy, dazzling, bright, foggy, gigantic",
    "title": "Visual Sensory Word"
  },
  "weakverb": {
    "instances": [
      {
        "index": 12,
        "token": "work",
        "end": 80,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 76
      },
      {
        "index": 18,
        "token": "do",
        "end": 109,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 107
      },
      {
        "index": 30,
        "token": "work",
        "end": 171,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 167
      },
      {
        "index": 43,
        "token": "had",
        "end": 233,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 230
      },
      {
        "index": 46,
        "token": "need",
        "end": 253,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 249
      },
      {
        "index": 50,
        "token": "work",
        "end": 276,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 272
      },
      {
        "index": 54,
        "token": "felt",
        "end": 294,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 290
      },
      {
        "index": 60,
        "token": "felt",
        "end": 321,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 317
      },
      {
        "index": 77,
        "token": "see",
        "end": 398,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 395
      },
      {
        "index": 81,
        "token": "need",
        "end": 417,
        "data": {
          "remove": true,
          "suggestion": "Replace with more specific verb."
        },
        "start": 413
      }
    ],
    "description": "These words don't trigger vivid imagery in reader's heads and usually require additional words to deliver a meaningful message. Substituting weak verbs with more specific ones will liven most texts.",
    "title": "Weak Verb"
  },
  "touch": {
    "instances": [],
    "description": "Words related to touch describe textures. You can use them to describe feelings and abstract concepts, too: gritty, creepy, slimy, fluff, sticky.",
    "title": "Touch Sensory Word"
  },
  "longsents": {
    "instances": [],
    "description": "Sentences with 40 or more words. They might be too hard to understand. If so, break them down into several smaller sentences.",
    "title": "Extra Long Sentences"
  },
  "motion": {
    "instances": [],
    "description": "By using active words or describing movement, you help your readers experience your words. For instance: vibrating, soaring, mind-boggling, staggering, bumpy.",
    "title": "Motion Sensory Word"
  },
  "adverb": {
    "instances": [
      {
        "index": 9,
        "token": "continuously",
        "end": 60,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 48
      },
      {
        "index": 19,
        "token": "better",
        "end": 116,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 110
      },
      {
        "index": 33,
        "token": "enough",
        "end": 186,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 180
      },
      {
        "index": 42,
        "token": "always",
        "end": 229,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 223
      },
      {
        "index": 51,
        "token": "only",
        "end": 281,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 277
      },
      {
        "index": 52,
        "token": "when",
        "end": 286,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 282
      },
      {
        "index": 57,
        "token": "Only",
        "end": 308,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 304
      },
      {
        "index": 58,
        "token": "when",
        "end": 313,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 309
      },
      {
        "index": 63,
        "token": "Only",
        "end": 336,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 332
      },
      {
        "index": 64,
        "token": "when",
        "end": 341,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 337
      },
      {
        "index": 76,
        "token": "clearly",
        "end": 394,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 387
      },
      {
        "index": 78,
        "token": "now",
        "end": 402,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 399
      },
      {
        "index": 98,
        "token": "truly",
        "end": 497,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 492
      },
      {
        "index": 136,
        "token": "Also",
        "end": 708,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 704
      },
      {
        "index": 146,
        "token": "else",
        "end": 757,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 753
      },
      {
        "index": 157,
        "token": "rather",
        "end": 794,
        "data": {
          "remove": true,
          "suggestion": "Pick a more accurate verb"
        },
        "start": 788
      }
    ],
    "description": "You can almost always remove the adverb and pick a more accurate verb.",
    "title": "Adverb"
  },
  "auditory": {
    "instances": [
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.86376315452509744,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "snappy"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.79475575829936307,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "raucous"
        },
        "start": 200
      },
      {
        "index": 37,
        "token": "judgmental",
        "end": 210,
        "data": {
          "similarity": 0.81136912064415434,
          "remove": false,
          "suggestion": "Try replacing with following word",
          "replace": "inaudible"
        },
        "start": 200
      }
    ],
    "description": "Words related to hearing describe sounds. For instance: crashing, thumping, piercing, tingling, squeaky. Often these words mimic sounds.",
    "title": "Auditory Sensory Word"
  },
  "passive": {
    "instances": [
      {
        "start": 136,
        "token": "Also, everything you make will be criticized, or else ignored.",
        "end": 150,
        "data": {
          "remove": false,
          "suggestion": "Restructure to passive voice"
        }
      },
      {
        "start": 156,
        "token": "I’d rather be criticized than ignored.",
        "end": 163,
        "data": {
          "remove": false,
          "suggestion": "Restructure to passive voice"
        }
      }
    ],
    "description": "Often, sentences in passive voice sound weak and can be confusing. If so, rephrase the sentence in active voice.",
    "title": "Passive Voice"
  },
  "fuzzysents": {
    "instances": [],
    "description": "Sentences with subject and verb distant from each other. Bring the subject and verbs to start of the sentence if possible.",
    "title": "Vague Sentences"
  },
  "modal": {
    "instances": [
      {
        "index": 17,
        "token": "can",
        "end": 106,
        "start": 103,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 69,
        "token": "would",
        "end": 361,
        "start": 356,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 75,
        "token": "can",
        "end": 386,
        "start": 383,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 96,
        "token": "can",
        "end": 485,
        "start": 482,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 104,
        "token": "won’t",
        "end": 532,
        "start": 527,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 122,
        "token": "might",
        "end": 641,
        "start": 636,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 127,
        "token": "will",
        "end": 669,
        "start": 665,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      },
      {
        "index": 141,
        "token": "will",
        "end": 734,
        "start": 730,
        "data":{"suggestion":"Weakens statement. Remove or replace with stronger verb","remove":true}
      }
    ],
    "description": "Verb modifiers signifying ability or necessity. They can weaken statements by making them uncertain or too radical.",
    "title": "Modal"
  },
  "expletives": {
    "instances": [
      {
        "start": 0,
        "token": "It’s about all things being equal exhausting to continuously criticize your work, telling yourself you can do better.",
        "end": 21,
        "data": {
          "remove": false,
          "suggestion": "Restructure the sentence."
        }
      },
      {
        "start": 21,
        "token": "It’s exhausting to keep on thinking whether your work is good enough for people’s judgmental eyes.  ",
        "end": 41,
        "data": {
          "remove": false,
          "suggestion": "Restructure the sentence."
        }
      }
    ],
    "description": "When it, here, and there refer to nouns later in the sentence or to something unnamed, they weaken your writing by shifting emphasis away from the true drivers of your sentences.",
    "title": "Grammar Expletives"
  }
}

export default response;